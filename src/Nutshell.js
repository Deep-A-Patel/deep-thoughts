...
class Nutshell extends Component {
  ...
  register = (username, email, password) => {
    ...
    API.getAllUsers().then(users => {
      if (users.find(user => username === user.username || email === user.email) !== undefined) { 
    ...
    return (
      <CustomTheme>
        ...
        { this.state.isUserLoggedIn ? (
          <div className="nutshell-contentContainer" style={{ height: "calc(100% - 72px)", marginTop: "72px" }}>
            <Sidebar loggedIn={this.state.isUserLoggedIn} />
            <div style={{ marginLeft: "200px", width: "100%", marginTop: "calc(72px + 1rem)" }}>
              <ApplicationViews loggedIn={this.state.isUserLoggedIn} />
            </div>
          </div>
          ) : (
          <div style={{ marginTop: "calc(72px + 1rem)" }}>
            <ApplicationViews loggedIn={this.state.isUserLoggedIn} login={this.login} register={this.register} />
          </div> 
          )
        }
      </CustomTheme>
